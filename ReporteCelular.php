...
$sPageTitle = gettext("Reporte Celular");

//Obtener el PersonID fuera de la cadena de consulta
...
// Seguridad: El usuario debe tener el permiso Agregar o editar registros para usar este formulario de esas maneras
// Limpiar el manejo de errores: (como alguien que escribe una URL incorrecta? PersonID = manualmente)
if ($iPersonID > 0)
{
...
// Obtener matriz de lista de seguridad de campo
...
//Obtener la lista de campos de persona personalizada
...
//¿Es este el segundo pase?
if (isset($_POST["PersonSubmit"]) || isset($_POST["PersonSubmitAndAdd"]))
{
	//Obtener todas las variables del objeto request y asignarlas localmente
	...
	// Las cosas de la dirección de la persona se suele presionar en favor de la dirección de la familia Info
	...
	// bevand10 2012-04-26 Añadir soporte para mayúsculas ZIP - controlado por administrador vía cfg param
	...
	// Obtener el país de su familia en caso de que el país de la persona no haya sido ingresado
	/* if ($iFamily > 0) {
	...
	//Valide el apellido. Si la familia seleccionada, pero no el apellido, heredar de la familia.
	...
	// Si ingresaron una fecha completa, vea si es válida
	...
	// Validar la fecha del amigo si se ingresó
	...
	// Validar la fecha de afiliación si se ingresó
	...
	// Validar todos los campos personalizados
/* 	$aCustomData = array();
	...
	//Si no hay errores, entonces vamos a actualizar
	if (!$bErrorFlag)
	{
 ...
 */
		...
		// Nueva persona (agragar)
		if ($iPersonID < 1) {
			$iEnvelope = 0;

			$sSQL = "INSERT INTO person_per (per_Title, per_FirstName,  per_LastName,  per_Gender, per_Address1, per_Address2, per_City, per_State, per_Zip, per_Country, per_HomePhone, per_WorkPhone, per_CellPhone, per_Email, per_WorkEmail, per_BirthMonth, per_BirthDay, per_BirthYear, per_Envelope, per_fam_ID, per_fmr_ID, per_MembershipDate, per_cls_ID, per_DateEntered, per_EnteredBy, per_FriendDate, per_Flags ) 
			         VALUES ('" . $sTitle . "','" . $sFirstName . "','" . $sLastName . "'," . $iGender . ",'" . $sAddress1 . "','" . $sAddress2 . "','" . $sCity . "','" . $sState . "','" . $sZip . "','" . $sCountry . "','" . $sHomePhone . "','" . $sWorkPhone . "','" . $sCellPhone . "','" . $sEmail . "','" . $sWorkEmail . "'," . $iBirthMonth . "," . $iBirthDay . "," . $iBirthYear . "," . $iEnvelope . "," . $iFamily . "," . $iFamilyRole . ",";
			if ( strlen($dMembershipDate) > 0 )
		...
		// Si se trata de una nueva persona, recuperar la clave e insertar una fila en blanco en la tabla person_custom
		/* if ($bGetKeyBack)
		{
      ...
      $noteService->addNote($iPersonID, 0, 0, "Updated", "edit");
		...
		// Actualizar los campos de persona personalizada.
		/* if ($numCustomFields > 0)
		{
			...
			// Cortar los últimos 2 caracteres (coma y espacio) añadidos en la última iteración de bucle.
			if ($sSQL > "") {
...
//Obtener clasificaciones para el drop-down
$sSQL = "SELECT * FROM list_lst WHERE lst_ID = 1 ORDER BY lst_OptionSequence";
$rsClassifications = RunQuery($sSQL);

//obtener familias para el drop-down
$sSQL = "SELECT * FROM family_fam ORDER BY fam_Name";
$rsFamilies = RunQuery($sSQL);

//obtener roles de familia para el drop-down
...
?>
<form method="post" action="ReporteCelular.php?PersonID=<?= $iPersonID ?>" name="ReporteCelular" >
	<div class="alert alert-info alert-dismissable">
		<i class="fa fa-info"></i>
		<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
		<strong><span style="color: red;"><?= gettext("Texto Rojo") ?></span></strong> <?php echo gettext("indica los elementos heredados del registro familiar asociado.");?>
	</div>
	<?php if ( $bErrorFlag ) { ?>
	...
	<div class="box box-info clearfix">
		<div class="box-header">
			<h3 class="box-title"><?= gettext("Martes de Planeación") ?></h3>
		...
		<div class="box-body">
			<div class="form-group">
				<div class="row">
					<div class="col-md-2">
						...
						<select name="Gender" class="form-control">
							<option value="0"><?= gettext("Sexo") ?></option>
							<option value="0" disabled>-----------------------</option>
							<option value="1" <?php if ($iGender == 1) { echo "selected"; } ?>><?= gettext("Hombre") ?></option>
							<option value="2" <?php if ($iGender == 2) { echo "selected"; } ?>><?= gettext("Mujer") ?></option>
						</select>
					</div>
					<div class="col-md-3">
						<label for="Title"><?= gettext("AnteNombre:") ?></label>
						<input type="text" name="Title" id="Title" value="<?= htmlentities(stripslashes($sTitle),ENT_NOQUOTES, "UTF-8") ?>" class="form-control" placeholder="<?= gettext("Hno., Hna., Niño., Rev., Joven.,") ?>">
					</div>
				</div>
				<p/>
				<div class="row">
					<div class="col-md-4">
						<label for="FirstName"><?= gettext("Nombres:") ?></label>
					...
					<div class="col-md-4">
						<label for="LastName"><?= gettext("Apellidos:") ?></label>
				...
				<div class="row">
					<div class="col-md-2">
						<label><?= gettext("Mes:") ?></label>
						<select name="BirthMonth" class="form-control">
							<option value="0" <?php if ($iBirthMonth == 0) { echo "selected"; } ?>><?= gettext("Seleccionar") ?></option>
					...
					<div class="col-md-2">
						<label><?= gettext("Dia:") ?></label>
						<select name="BirthDay" class="form-control">
							<option value="0"><?= gettext("Selecciona Dia") ?></option>
							<?php for ($x=1; $x < 32; $x++) {
	...
	<div class="box box-info clearfix">
		<div class="box-header">
			<h3 class="box-title"><?= gettext("Información Familia") ?></h3>
			<div class="pull-right"><br/>
				<input type="submit" class="btn btn-primary" value="<?= gettext("Guardar") ?>" name="PersonSubmit">
			</div>
		</div><!-- /.box-header -->
		<div class="box-body">
			<div class="form-group col-md-3">
				<label><?= gettext("Rol de Familia:") ?></label>
				<select name="FamilyRole" class="form-control">
					<option value="0"><?= gettext("Inasignado") ?></option>
					<option value="0" disabled>-----------------------</option>
			...
			<div class="form-group col-md-6">
				...
				<select name="Family" size="8" class="form-control">
					<option value="0" selected><?= gettext("Inasignado a alguna familia") ?></option>
					<option value="-1"><?= gettext("Crear nueva familia (usando Apellidos)") ?></option>
					<option value="0" disabled>-----------------------</option>
	...
	<div class="box box-info clearfix">
		<div class="box-header">
			<h3 class="box-title"><?= gettext("Información del Contacto") ?></h3>
			<div class="pull-right"><br/>
				<input type="submit" class="btn btn-primary" value="<?= gettext("Guardar") ?>" name="PersonSubmit">
			</div>
		</div><!-- /.box-header -->
		<div class="box-body">
			...
			<div class="row">
				...
				<div class="form-group col-md-3">
					...
					<?php
					if ($bFamilyWorkPhone)
						echo "<span style=\"color: red;\">" . gettext("Telefono de Trabajo:") . "</span>";
					else
	...
	<div class="box box-info clearfix">
		<div class="box-header">
			<h3 class="box-title"><?= gettext("Informacion de membresia") ?></h3>
			<div class="pull-right"><br/>
		...
		<div class="box-body">
			<div class="row">
				...
				<div class="form-group col-md-4">
					<label><?= gettext("Fecha de Bautismo:") ?></label>
					<div class="input-group">
						<div class="input-group-addon">
				...
				<?php } ?>
				<div class="form-group col-md-4">
					<label><?= gettext("Fecha de Miembro:") ?></label>
					<div class="input-group">
						<div class="input-group-addon">
							<i class="fa fa-calendar"></i>
						</div>
			...
			<div class="form-group col-md-4">
				<label><?= gettext("Clasificacion:") ?></label>
