...
$sPageTitle = gettext("Agregar persona a celula");
require "Include/Header.php";
?>

<script src="js/RPCDummyAjax.js"></script>

<p align="center"><?= gettext("Seleccione el grupo al que desea agregar esta persona:") ?></p>
<form method="post" action="PersonToGroup.php?PersonID=<?= $iPersonID ?>">
  <input type="hidden" name="prevquery" value="<?= $sPreviousQuery ?>">
  <table align="center">
    <tr>
      <td class="LabelColumn"><?= gettext("Seleccionar Celula:") ?></td>
      <td class="TextColumn">
        <?php
// Create the group select drop-down
        echo "<select id=\"GroupID\" name=\"GroupID\" onChange=\"UpdateRoles();\"><option value=\"0\">" . gettext("Ninguna") . "</option>";
    ...
    <tr>
      <td class="LabelColumn"><?= gettext("Selecciona Tipo de Miembro:") ?></td>
      <td class="TextColumn"> 
        <select name="GroupRole" id="GroupRole">
          <option><?= gettext("No celula Seleccionada") ?></option>
        </select>
      </td>
    </tr>
  </table>
  <p align="center">
    <BR>
    <input type="submit" class="btn" name="Submit" value="<?= gettext("Agregar a Celula") ?>">
    <BR><BR>
  </p>
</form>
